...
def chebyshevCoefficient(chebyshevOrder, inputNumber, outputNumber):
    ...
    for i in range(chebyshevOrder):
        initial = tf.random.truncated_normal(shape=[inputNumber, outputNumber], mean=0, stddev=0.05)
...
def globalPooling(gcnOutput):
    ...
    mean, var = tf.nn.moments(x=gcnOutput, axes=[1])
    max_f = tf.reduce_max(input_tensor=gcnOutput, axis=[1])
