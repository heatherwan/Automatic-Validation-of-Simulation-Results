...
def train():
    with tf.Graph().as_default():  # for define a new graph to put in every element
        ...
        # ================================Load data===============================
        inputTrain, trainother, trainLabel, inputTest, testother, testLabel = read_data.load_data()
        ...
        for epoch in range(para.max_epoch):
            ...
            log_string(f'learningRate:\t{learningRate:.6f}')

            inputtrainall = np.dstack((inputTrain, trainother))

            train_average_loss, train_average_acc = model.trainOneEpoch(inputtrainall,
                                                                        scaledLaplacianTrain,
                                                                        trainLabel,
                                                                        para, sess,
                                                                        trainOperation,
