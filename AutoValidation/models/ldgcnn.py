...
def get_loss(pred, label, end_points):
    ...
    # make coarse label
    if para.outputClassN == 5:
        gather_idx = tf.constant([0, 1, 1, 1, 1])
    else:
        gather_idx = tf.constant([0, 1, 1, 1])
    coarse_label = tf.transpose(tf.math.unsorted_segment_sum(tf.transpose(labels), gather_idx, num_segments=2))
