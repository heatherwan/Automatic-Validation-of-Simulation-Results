...
def CalcVertexNormals(FV, N):
    ...
    VertexNormals, up, vp = np.zeros((np.shape(FV.vertices)[0], 3)), \
...
def getPrincipalCurvatures(FV, VertexSFM):
    """
    Calculates the principal curvatures and principal directions
    INPUT :
    FV : triangular mesh
